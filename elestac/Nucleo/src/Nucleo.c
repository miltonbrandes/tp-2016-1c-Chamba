...
void datosEnSocketReceptorCPU(int nuevoSocketConexion) {
	...
	uint32_t id;
	uint32_t operacion;
	int bytesRecibidos = recibirDatos(nuevoSocketConexion, &buffer, &operacion,
			&id);
	...
	while (1) {
		...
		if (resultadoSelect == 0) {
			...
			if (FD_ISSET(socketReceptorCPU, &tempSockets)) {
				...
				char *buffer;
				uint32_t operacion;
				int bytesRecibidos = recibirDatos(nuevoSocketConexion, &buffer,
						&operacion, &id);
				if (bytesRecibidos < 0) {
					log_info(ptrLog,
				...
				for (socketFor = 0; socketFor < (socketMaximo + 1);
					...
					if (FD_ISSET(socketFor, &tempSockets)) {
						bytesRecibidos = recibirDatos(socketFor, &buffer,
								&operacion, &id);
						if (bytesRecibidos > 0) {
							if (id == CPU) {
...
t_pcb* crearPCB(char* programa, int socket) {
	...
	log_info(ptrLog,
			"Enviamos el 1er pedido a la UMV, para crear el espacio de codigo literal");
	...
	if (rtaOp < 0) {
		...
		log_error(ptrLog,
				"No se pudo crear el espacio de codigo literal para el Programa %i",
				pcb->pcb_id);
	...
	log_debug(ptrLog,
			"Se ha escrito correctamente sobre las paginas de codigo");
	free(envioBytes.buffer);
	free(rtaEnvio);

	log_debug(ptrLog,
			"Rellenamos la estructura del pcb con las direcciones devueltas por la UMC y completamos"
