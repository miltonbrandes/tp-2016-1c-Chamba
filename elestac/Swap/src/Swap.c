...
int interpretarMensajeRecibido(char* buffer,int op, int socketUMC, t_list* listaDeLibres, t_list* listaDeOcupados){
	uint32_t resultado;
	...
	switch(op){
		case NUEVOPROGRAMA:
			...
			log_info(ptrLog, "UMC Solicita la creacion de espacio para el Proceso %d", checkEspacio->pid);
			resultado = asignarMemoria(checkEspacio->pid, checkEspacio->cantidadDePaginas, listaDeLibres, listaDeOcupados);
			if (!resultado)
			{
				buffer_tamanio = serializarUint32(ERROR);
				int enviado=enviarDatos(socketUMC, buffer_tamanio->buffer, buffer_tamanio->tamanioBuffer, RECHAZAR_PROCESO_A_UMC, SWAP);
				if(enviado <= 0)
				{
				...
				int bytesEnviados = enviarDatos(socketUMC, buffer_tamanio->buffer, buffer_tamanio->tamanioBuffer, ACEPTAR_PROCESO_A_UMC, SWAP);
				if(bytesEnviados <= 0) {
...
int asignarMemoria(uint32_t pid, uint32_t cantidad_paginas, t_list* listaDeLibres, t_list* listaDeOcupados) { //le damos memoria al proceso nuevo
	log_info(ptrLog, "Comienzo a chequear si hay espacio para el Proceso %d", pid);
	...
	if(hueco == NULL)
	{
		log_info(ptrLog, "Necesito desfragmentar / compactar memoria para alojar el Proceso %d", pid);
	...
	log_info(ptrLog, "Se ubicó correctamente el proceso %d partiendo del byte %d y ocupando %d bytes.", pid, (comienzoProceso*tamanoPagina), (cantidad_paginas*tamanoPagina));
	return 1;
}
...
int main() {
	if (init()) {
		...
		if (exito_creacion_archivo) {
			log_info(ptrLog, "Se creo correctamente el archivo de control de memoria");
		} else {
